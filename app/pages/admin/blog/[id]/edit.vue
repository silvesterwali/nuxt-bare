...
const { data: post, error: fetchError, pending } = await useFetch(`/api/admin/blog/${postId.value}`)
...
const formPost = computed(() => {
  ...
  return {
    ...
    shortDescription: p.shortDescription?.[locale.value] || p.shortDescription?.en || '',
...
async function handleSubmit(formData: any) {
  ...
  try {
    ...
    const updateData = {
      ...
      shortDescription: { ...post.value?.data?.shortDescription, [locale.value]: formData.shortDescription },
      content: { ...post.value?.data?.content, [locale.value]: formData.content },
