...
export default defineEventHandler(async (event) => {
  ...
  const { type, privacy, page, limit } = await useValidatedQuery(event, querySchema);

  const { page: validPage, limit: validLimit } = validatePaginationParams({ page, limit });

  // Get user media
  const media = await getUserMedia(session.user.id, type, privacy, validPage, validLimit);
